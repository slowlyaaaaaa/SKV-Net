...
for name in anno_name_list:  # 遍历有结节图的名字
    ...
    ct_image1[ct_image1>1]=1  # LUNA16提供的肺部掩膜分左右肺，左肺为3右肺为4，我们需要统一为1，非肺部区域为0
    ...
    print(msk.shape)  # 这俩一样大
       # LUNA16竞赛中常用来做归一化处理的阈值集是-1000和400（最接近肺质）
    max_num = 400  # 阈值最高
    min_num = -1000  # 阈值最低
    ...
    if not os.path.exists(output_path+fengefu+"bbox_image"):#预处理之后的图（用来实验的）
        os.mkdir(output_path+fengefu+"bbox_image")
    if not os.path.exists(output_path+fengefu+"bbox_mask"):#预处理的标签（带轮廓的标签）
