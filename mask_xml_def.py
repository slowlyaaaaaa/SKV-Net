...
def get_raw_label(img_name, img, annos, origin,
                  zheng):  # 进行单图的坐标转换，把结节坐标 从世界坐标 准换到 图像坐标 （[1,1,1]像距） 输入为：图的名字，图，LUNA6中的annotations，像素间隔，是否翻转    第四步
    ...
    for one_annos_list in annos_these_list:  # {坐标转化}打开第一个结节数据["名字",x,y,z,diam]
        print("one_annos_list:", one_annos_list)  # 打印出第一个结节数据["名字",x,y,z,diam]，此时是世界坐标
        w_center = [one_annos_list[1], one_annos_list[2], one_annos_list[3]]  # w_center为世界坐标的xyz（1：x，2：y，3：z）
        ...
        if zheng is False:  # 如果是反的，由于图反过来了，结节坐标也要反过来
            v_center = [img.shape[2] - 1 - v_center[0], img.shape[1] - 1 - v_center[1],
                        v_center[2]]  # img.shape[2]就是x的总长，-1是因为从0开始，-v_center[0]是减去x的坐标，也即翻转， * z轴不需翻转*
        diam = one_annos_list[4]  # 直径（4：z）
        print("结节直径", diam)
        one_annos = []
    ...
    for i in range(10):  # 默认你10个subset都下完了      #第一步得到所有图的名字
        ...
        for ii in file_list:  # 遍历如 subset0 内所有文件
            if len(ii.split(".m")) == 2:  # 如果文件名是mhd文件的话
                ...
                ct_image_path = find_mhd_path(name)  # 把文件名拿去找对应的mhd文件的绝对地址   看第二步
                numpyImage, origin, spacing, fanzhuan = read_data(ct_image_path)  # 读取这个mhd文件（输出返回的位置在这）  看第三步
                one_annos = get_raw_label(name, numpyImage, c, origin, fanzhuan)  # 进行坐标变换     看第四步
                d.append([name, one_annos])  # 把变换后的 ["名字",[[结节1],[结节2],...]] 添加到d里（加了名字补齐上一步没完成的）
...
def name(xml_path):  # 从xml文件中取得name
    ...
    dom = xml.dom.minidom.parse(xml_path)  # 读取xml文件（xml.dom.minidom.parse读取xml文件的工具）
    ...
    return child_value  # name 遍历xml文件找name文件一样的找出地址
...
def point(xml_path, origin2):  # 需要xml_path 和 图像的原点坐标origin的z轴坐标
        ...
        for i in child:  # 遍历该（unblindedReadNodule）下的所有点
            ...
            if id1:  # 如果（noduleID）的值不为空
                ...
                for r in one_all_iou:  # 遍历这些（roi）点
                    z = r.getElementsByTagName(
                        "imageZposition")  # 取得该（unblindedReadNodule）下的 （noduleID）下的（roi）下的（imageZposition）
                    z1 = float(z[0].childNodes[
                                   0].nodeValue) - origin2  # 取得图像坐标的z轴  ， 即 z轴坐标-原点的z轴坐标。  其中 图像在x、y轴不需要变化，在标注时就是按照图像坐标标注的
    ...
    return a  # 返回该案例图的 所有z轴和z轴上点位 的列表  [  [z1,[[x1,y1],[x2,y2],...]],  [z2,[[x1,y1],[x2,y2],...]],  ...]（读取文件获得一个个坐标）


def read_data(mhd_file):  # 读取图像数据（包括图，坐标原点，像素间隔，是否需要翻转） 第三步
    with open(mhd_file) as f:  # 打开MHD文件
        mhd_data = f.readlines()  # 对于打开的MHD文件着行读取
    ...
    itkimage = sitk.ReadImage(mhd_file)  # 读取mhd文件（MHD不是图，RAW是图，MHD是文件）
    numpyImage = sitk.GetArrayFromImage(itkimage)  # etArrayFromImage（读图工具）从mhd读取到raw，也就是图
    print("读取数据，读取的图片大小（zyx）：", numpyImage.shape)  # 打印图有多大 深 depth  *  宽 width  *  高 height
    origin = itkimage.GetOrigin()  # 从mhd读取到origin，也就是原点坐标（读取原点坐标）
    print("读取数据，读取的坐标原点（xyz）：", origin)  # 坐标原点   x,y,z
    spacing = itkimage.GetSpacing()  # 从mhd读取到spacing，也就是像素间隔（读取像素间隔）
    print("读取数据，读取的像素间隔（xyz）：", spacing)  # 像素间隔   x,y,z
    return numpyImage, origin, spacing, zheng  # （原图，原点坐标，像素间隔，正反面）判断正反和上面的IF语句对比即可（FALSE就是要翻转）   返回上面的
...
def for_one_(name, wrong):  # 一个处理每张图的函数，输入名字＋一个空列表， 输出 上色好的mask + mhd的绝对地址用于后续存放mask用 + 出错的图的名字
    ...
    for i in xml_path_list:  # 取得xml文件的绝对地址，
        list1 = point(i, origin[2])  # 在这个绝对地址内获取所有[ [ z层 ， 点 ]，[ z层 ，点 ] ， ... ]（i为xml文件的绝对地址）
        print(len(list1))  # 共多少层
        for ii in list1:  # 遍历所有层
            ceng = ii[0]  # ceng为z轴坐标
            print("ceng", ceng)  # 打印层
            pts = ii[1]  # 该层的所有点位  [[x1,y1],[x2,y2],...]
            color = 1  # (0, 255, 0)（二分类不是0就是1）
            ...
            mm[int(ceng / spacing[2] - 1), :, :] = cv.drawContours(mm[int(ceng / spacing[2] - 1), :, :], [pts], -1,
                                                                   color=color, thickness=-1)  # 取出这一层，开始染色填充
            mm[int(ceng / spacing[2] - 1), :, :] = scipy.ndimage.binary_fill_holes(mm[int(ceng / spacing[2] - 1), :, :],
...
##遍历数据集subset0到subset9所有名字
##LUNA16数据集的subset文件包含MHD和RAw文件
