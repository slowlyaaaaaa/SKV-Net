    ...
    for i in range(10):  # 默认你10个subset都下完了
        ...
        for ii in file_list:  # 遍历如 subset0 内所有文件
            if len(ii.split(".m")) == 2:  # 如果文件名是mhd文件的话
                ...
                numpyImage, origin, spacing, fanzhuan = read_data(ct_image_path)  # 读取这个mhd文件（输出返回的位置在这）
...
def read_data(mhd_file):  # 读取图像数据（包括图，坐标原点，像素间隔，是否需要翻转）
    ...
    return numpyImage,origin,spacing,zheng   #（原图，原点坐标，像素间隔，正反面）判断正反和上面的IF语句对比即可（FALSE就是要翻转）
